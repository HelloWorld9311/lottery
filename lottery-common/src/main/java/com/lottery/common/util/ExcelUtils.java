...
public class ExcelUtils {
     ...
     * @return 返回workbook
     */
    private static <T> Workbook writeToWorkbook(Class<T> clazz, List<T> list, String specialTitleContent) {
        ...
        for (int i = 0, length = list.size(); i < length; i++) {
            ...
            if (rowNumber == 0 || rowNumber % DEFAULT_EXCEL_ROW_NUM == 0) {
                ...
                if( !StringUtils.isBlank(specialTitleContent) ){ rowNumber ++; }
                //设置正常的表头
     ...
     * @throws Exception 异常
     */
    private static <T> Workbook writeToWorkbook2(Class<T> clazz, List<T> list, String specialTitleContent) throws Exception {
    ...
    private static CellStyle newTableSpecialHeaderStyle(Workbook wb, String specialTitleContent) {
        if( StringUtils.isBlank(specialTitleContent) ){ return null; }
    ...
    private static void setExcelSpecialTitleDatas(Sheet sheet, String specialTitleContent, List<String> headerNames, CellStyle headerStyle, Short rowHeight, Map<Integer, Integer> columnWidths, int rowNumber) {
        if( StringUtils.isBlank(specialTitleContent) ){ return; }
